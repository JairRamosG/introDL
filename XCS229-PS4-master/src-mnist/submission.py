...
def forward_prop(data, labels, params):
    ...
    loss = -np.sum(labels * np.log(y_hat + 1e-12)) / data.shape[0]
...
def backward_prop(data, labels, params, forward_prop_func):
    ...
    h, y_hat, loss = forward_prop_func(data, labels, params)

    m = data.shape[0]

    # Derivada de loss respecto a la salida
    dz2 = (y_hat - labels) / m

    dW2 = np.dot(h.T, dz2)
    db2 = np.sum(dz2, axis=0)

    dz1 = np.dot(dz2, W2.T) * h * (1 - h)
